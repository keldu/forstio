...
void UnixIoStream::readStep(){
	while(!read_tasks.empty()){
		...
		}else if(static_cast<size_t>(n) >= task.min_length && static_cast<size_t>(n) <= task.max_length){
			if(read_done){
...
void UnixIoStream::writeStep(){
	while(!write_tasks.empty()){
		...
		if(n<0){
			...
			task.buffer = reinterpret_cast<const uint8_t*>(task.buffer) + static_cast<size_t>(n);
...
UnixIoStream::UnixIoStream(UnixEventPort& event_port, int file_descriptor, int fd_flags, uint32_t event_mask):
...
UnixServer::UnixServer(UnixEventPort &event_port, int file_descriptor, int fd_flags):
...
void UnixServer::notify(uint32_t mask){
	if(mask & EPOLLIN){
		if(accept_feeder){
			...
			int accept_fd = ::accept4(fd(), reinterpret_cast<struct ::sockaddr*>(&address), &address_length, SOCK_NONBLOCK | SOCK_CLOEXEC);
			if(accept_fd < 0){
				return;
			}
			auto fd_stream = heap<UnixIoStream>(event_port, accept_fd, 0, EPOLLIN | EPOLLOUT);
...
namespace {
bool beginsWith(const std::string_view& viewed, const std::string_view& begins){
	return viewed.size() >= begins.size() && viewed.compare(0, begins.size(), begins) == 0;
}
}
...
Conveyor<Own<IoStream>> UnixNetworkAddress::connect(){
	...
	return heap<UnixIoStream>(event_port, fd, 0, EPOLLIN | EPOLLOUT);
}
...
Own<NetworkAddress> UnixAsyncIoProvider::parseAddress(const std::string &path, uint16_t port_hint) {
	EventPort* port = event_loop.eventPort();
	if(!port){
		return nullptr;
	}
	...
	std::list<SocketAddress> addresses = SocketAddress::parse(addr_view, port_hint);

	return heap<UnixNetworkAddress>(*port, *this, path, port_hint, std::move(addresses));
}
